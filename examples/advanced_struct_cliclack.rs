...
// 1) build an example: cargo build --example advanced_struct_cliclack
// 2) go to the `examples` folder: cd target/debug/examples
// 3) run an example: ./advanced_struct_cliclack (without parameters) => entered interactive mode
//                    ./advanced_struct_cliclack --age-full-years 30 --first-name QWE --second-name QWERTY --favorite-color red =>
//                                    => cli_args: CliArgs { age: Some(30), first_name: Some("QWE"), second_name: Some("QWERTY"), favorite_color: Some(Red) }
//                    ./advanced_struct_cliclack --first-name QWE --second-name QWERTY --favorite-color red =>
//                                    => cli_args: CliArgs { age: None, first_name: Some("QWE"), second_name: Some("QWERTY"), favorite_color: Some(Red) }
// To learn more about the parameters, use "help" flag: ./advanced_struct_cliclack --help
...
impl Args {
    fn input_second_name(_context: &()) -> color_eyre::eyre::Result<Option<String>> {
        match input("Input second name").interact() {
    ...
    fn input_favorite_color(_context: &()) -> color_eyre::eyre::Result<Option<ColorPalette>> {
        let variants = ColorPaletteDiscriminants::iter()
            .map(|v| (v, v, ""))
            .collect::<Vec<_>>();
        let selected = select("What color is your favorite?")
            .items(&variants)
            .interact()?;
        match selected {
