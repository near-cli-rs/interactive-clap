...
// 1) build an example: cargo build --example struct_with_context_cliclack
// 2) go to the `examples` folder: cd target/debug/examples
// 3) run an example: ./struct_with_context_cliclack (without parameters) => entered interactive mode
//                    ./struct_with_context_cliclack account QWERTY => offline_args: Ok(OfflineArgs { account: Sender { sender_account_id: "QWERTY" } })
// To learn more about the parameters, use "help" flag: ./struct_with_context_cliclack --help
...
impl Sender {
    fn input_sender_account_id(
        ...
        match cliclack::input("What is the account ID?").interact() {
